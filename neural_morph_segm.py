...
class Partitioner:
    ...
    def check_params(self):
        ...
        if (self.early_stopping is not None and
            ...
            self.callbacks.append(EarlyStopping(patience=self.early_stopping, monitor="accuracy"))
...
def init(config_file_path: Path):

    config_file = CONFIG_PATH / config_file_path
    ...
    if inputs is not None:
        model_to_save = cls.train(inputs, targets, dev_inputs, dev_targets, model_file=params.get("model_file"))
...
def dumpModelPickle(cls: Partitioner):
    import pickle
    with open(DUMP_PATH / "output.pcl", "w") as f:
